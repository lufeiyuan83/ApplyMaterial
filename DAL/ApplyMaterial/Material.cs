...
{
    public class Material : IDAL.ApplyMaterial.IMaterial
    {
        ...
        public Model.ApplyMaterial.Material Select(long m_Id)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    Model.ApplyMaterial.Material material = new Model.ApplyMaterial.Material();
        ...
        public bool Add(Model.ApplyMaterial.Material m_Material)
        {
            ...
            { 
                ...
                return da.ExecuteCommand("Insert Into " + tableName + "(MaterialId,ProductionName,Specification,CreateDateTime,CreateUserID,LastUpdateDateTime,LastUpdateUserID,Remark) Values(@MaterialId,@ProductionName,@Specification,GetDate(),@CreateUserID,GetDate(),@LastUpdateUserID,@Remark)", da.sqlParameters)!=0;
            }
        }
        /// <summary>
        /// 是否已经存在该物料信息
