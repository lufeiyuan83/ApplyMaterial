...
{
    public class SysMenu : IDAL.Tenement.ISysMenu
    {
        private const string tableName = "Tenement.dbo.SysMenu";
        /// <summary>
        /// 查询系统菜单
        /// </summary>
        /// <returns></returns>
        ...
        public Model.Tenement.SysMenu Select(long m_Id)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    Model.Tenement.SysMenu sysMenu = new Model.Tenement.SysMenu();
