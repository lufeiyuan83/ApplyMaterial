...
{
    public class Login : IDAL.Tenement.ILogin
    {
        private const string tableName = "Tenement.dbo.Login";
        /// <summary>
        /// 查询Login
        /// </summary>
        ...
        public Model.Tenement.Login Select(string m_UserName)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    Model.Tenement.Login login = new Model.Tenement.Login();
                    login.TenementCode = dr["TenementCode"].ToString();
        ...
        public bool Add(Model.Tenement.Login m_Login)
        {
            ...
            {
                da.sqlParameters.Clear();
                da.sqlParameters.Add(da.CreateSqlParameter("@TenementCode", m_Login.TenementCode));
                ...
                return da.ExecuteCommand("Insert Into " + tableName + "(TenementCode,UserName,Password,LoginCount,IsLock,Validity,IsChange,LoginErrorCount) Values(@TenementCode,@UserName,@Password,@LoginCount,@IsLock,@Validity,@IsChange,@LoginErrorCount)", da.sqlParameters) != 0 ? true : false;
            }
        }
        /// <summary>
        /// 更新Login
        ...
        /// <returns>返回增加结果</returns>
        public bool Update(Model.Tenement.Login m_Login, string m_OldEmployeeID)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
