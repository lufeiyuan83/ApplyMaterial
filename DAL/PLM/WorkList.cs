...
namespace DAL.PLM
{
    public class WorkList :IDAL.PLM.IWorkList
    {
        ...
        /// </summary>
        /// <returns></returns>
        public Model.PLM.WorkList Select(long m_id)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    ...
                    workList.FlowGroup = Convert.ToInt32(dr["FlowGroup"]);
        ...
        /// </summary>
        /// <returns></returns>
        public Model.PLM.WorkList Search(string m_SystemCode,string m_TableName, string m_FormID)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
                ...
                if (dt != null && dt.Rows.Count>0)
                {
                    ...
                    workList.FlowGroup = Convert.ToInt32(dt.Rows[0]["FlowGroup"]);
        ...
        /// </summary>
        /// <returns></returns>
        public bool Add(Model.PLM.WorkList m_WorkList)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
                ...
                da.sqlParameters.Add(da.CreateSqlParameter("@FlowGroup", m_WorkList.FlowGroup));
                ...
                return da.ExecuteCommand("Insert Into " + tableName + "(TableName,FormId,SystemCode,Title,URL,FlowName,FlowGroup,Approver,Status,CreateUserID,CreateDateTime) Values(@TableName,@FormId,@SystemCode,@Title,@URL,@FlowName,@FlowGroup,@Approver,@Status,@CreateUserID,GetDate())", da.sqlParameters) != 0 ? true : false;
            }
        }
        /// <summary>
        /// 记录工作列表
        ...
        /// </summary>
        /// <returns></returns>
        public bool Add(List<Model.PLM.WorkList> m_WorkList)
        {
            using (DataAccess da = new DataAccess())
            {
                try
                {
                    ...
                    for (int i = 0; i < m_WorkList.Count; i++)
                    {
                        ...
                        da.sqlParameters.Add(da.CreateSqlParameter("@FlowGroup", m_WorkList[i].FlowGroup));
                        ...
                        da.ExecuteCommand("Insert Into " + tableName + "(TableName,FormId,SystemCode,Title,URL,FlowName,FlowGroup,Approver,Status,CreateUserID,CreateDateTime) Values(@TableName,@FormId,@SystemCode,@Title,@URL,@FlowName,@FlowGroup,@Approver,@Status,@CreateUserID,GetDate())", da.sqlParameters);
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public bool Update(Model.PLM.WorkList m_WorkList)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear(); 
                ...
                da.sqlParameters.Add(da.CreateSqlParameter("@FlowGroup", m_WorkList.FlowGroup));
                da.sqlParameters.Add(da.CreateSqlParameter("@Status", m_WorkList.Status));
                return da.ExecuteCommand("Update " + tableName + " Set TableName=@TableName,SystemCode=@SystemCode,Title=@Title,URL=@URL,FlowName=@FlowName,FlowGroup=@FlowGroup,Status=@Status Where FormId=@FormId", da.sqlParameters) != 0 ? true : false;
            }
        }
        /// <summary>
        /// 更新工作列表的状态
