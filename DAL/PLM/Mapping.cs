...
namespace DAL.PLM
{
    public class Mapping : IDAL.PLM.IMapping
    {
        ...
        public DataSet Select(string m_MapGroup, List<string> m_MapCode)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                { 
                    ...
                    for (int i = 0; i < m_MapCode.Count; i++)
                    { 
                        da.sqlParameters.Clear();
                        da.sqlParameters.Add(da.CreateSqlParameter("@MapGroup", m_MapGroup.Trim().ToUpper()));
                        da.sqlParameters.Add(da.CreateSqlParameter("@MapCode", m_MapCode[i].Trim().ToUpper()));
                        DataTable dt = da.GetDataTable("Select * From " + tableName + " Where Upper(MapGroup)=@MapGroup And Upper(MapCode)=@MapCode And IsDeleted=0 Order By MapCode,SortNumber", da.sqlParameters);
                        dt.TableName = m_MapCode[i];
                        ds.Tables.Add(dt);
                    }
        ...
        public DataTable Search(string m_MapGroup, string m_MappingName1, string m_MappingType1)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
                da.sqlParameters.Add(da.CreateSqlParameter("@MapGroup", m_MapGroup.Trim().ToUpper()));
                da.sqlParameters.Add(da.CreateSqlParameter("@MappingName1", m_MappingName1.Trim().ToUpper()));
                da.sqlParameters.Add(da.CreateSqlParameter("@MappingType1", m_MappingType1.Trim().ToUpper()));
                return da.GetDataTable("Select * From " + tableName + " Where Upper(MapGroup)=@MapGroup And Upper(MappingName1)=@MappingName1 And Upper(MappingType1)=@MappingType1 And IsDeleted=0 Order By MapCode,SortNumber", da.sqlParameters);
            }
        }
        /// <summary>
        /// 查询Mapping的扩展子项
        ...
        public Model.PLM.Mapping Select(long m_MappingID)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    Model.PLM.Mapping mapping = new Model.PLM.Mapping();
