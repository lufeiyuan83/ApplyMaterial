...
namespace DAL.PLM
{
    public class ApplyMaterial :IDAL.PLM.IApplyMaterial
    {
        ...
        public Model.PLM.ApplyMaterial Select(long m_Id)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    ...
                    applyMaterial.Rev = dr["Rev"].ToString().Trim();
        ...
        public List<Model.PLM.ApplyMaterial> Select(List<string> m_MaterialId)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                {
                    List<Model.PLM.ApplyMaterial> lstApplyMaterial = new List<Model.PLM.ApplyMaterial>();
                    for (int i=0;i< dt.Rows.Count;i++)
                    {
                        Model.PLM.ApplyMaterial applyMaterial = new Model.PLM.ApplyMaterial();
                        applyMaterial.Id = Convert.ToInt64(dt.Rows[i]["Id"]);
                        applyMaterial.Rev = dt.Rows[i]["Rev"].ToString().Trim();
        ...
        public DataTable Select(string m_Bucode, string m_MaterialId,string m_ProductionName,string m_Specification, string m_CreateUserID)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (!String.IsNullOrWhiteSpace(m_ProductionName))
                {
                    strWhere += " And Upper(a.ProductionName) Like '%" + m_ProductionName.Trim().ToUpper() + "%'";
                }
                if (!String.IsNullOrWhiteSpace(m_Specification))
                {
                    strWhere += " And Upper(a.Specification) Like '%" + m_Specification.Trim().ToUpper() + "%'";
                }
                if (!String.IsNullOrWhiteSpace(m_CreateUserID))
                {
                    da.sqlParameters.Add(da.CreateSqlParameter("@CreateUserID", m_CreateUserID.Trim().ToUpper()));
        ...
        public bool Add(Model.PLM.ApplyMaterial m_ApplyMaterial, List<Model.PLM.TipTopParameter> m_TipTopParameter,out string m_MaterialId)
        {
            using (DataAccess da = new DataAccess())
            {
                try
                {
                    ...
                    string strSN = "";
                    string strMaxMaterialId = m_ApplyMaterial.MaterialId;
                    m_MaterialId = m_ApplyMaterial.MaterialId;
                    if (intStartStarIndex != -1)
                    {
                        da.sqlParameters.Clear();
                        object objMaxMaterialId = da.GetScalar("Select Max(MaterialId) From " + tableName + " Where MaterialId Like '" + m_ApplyMaterial.MaterialId.Trim().Substring(0, intStartStarIndex) + "%' And MaterialId Like '%" + m_ApplyMaterial.MaterialId.Trim().Substring(intEndStarIndex + 1) + "'", da.sqlParameters);
                        if (objMaxMaterialId != null)
                        {
                            ...
                            if (Convert.ToInt32(strSN) < Convert.ToInt32(m_TipTopParameter[0].StartSN))
                                strSN = m_TipTopParameter[0].StartSN.PadLeft(strSN.Length, '0');

                            strMaxMaterialId = strMaxMaterialId.Substring(0, intStartStarIndex) + strSN + strMaxMaterialId.Substring(intEndStarIndex + 1, strMaxMaterialId.Length - intEndStarIndex - 1);
                    ...
                    da.sqlParameters.Add(da.CreateSqlParameter("@Rev", m_ApplyMaterial.Rev.Trim()));
                    ...
                    da.ExecuteCommand("Insert Into " + tableName + "(MaterialId,Rev,ProductionName,Specification,BuCode,Status,CreateDateTime,CreateUserID,LastUpdateUserID,LastUpdateDateTime,Remark) Values(@MaterialId,@Rev,@ProductionName,@Specification,@BuCode,@Status,GetDate(),@CreateUserID,@LastUpdateUserID,GetDate(),@Remark)", da.sqlParameters);
