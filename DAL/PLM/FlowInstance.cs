...
namespace DAL.PLM
{
    public class FlowInstance : IDAL.PLM.IFlowInstance
    {
        ...
        /// </summary>
        /// <returns></returns>
        public Model.PLM.FlowInstance Select(long m_id)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (dr != null)
                {
                    Model.PLM.FlowInstance flowInstance = new Model.PLM.FlowInstance();
                    flowInstance.Id = Convert.ToInt64(dr["id"]);
                    flowInstance.FlowName = dr["FlowName"].ToString().Trim();
                    flowInstance.FlowGroup = Convert.ToInt32(dr["FlowGroup"]);
        ...
        public DataTable Select(string m_FlowName, string m_FormID)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                if (!string.IsNullOrEmpty(m_FlowName))
                {
                    ...
                    strFilter += " And Upper(FlowName)=@FlowName";
                }
        ...
        public DataTable Select(string m_FlowName,string m_FormID,int m_Step)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                return da.GetDataTable("SELECT * FROM " + tableName + " Where IsDeleted=0 And Upper(FlowName)=@FlowName And FormID=@FormID And Step=@Step", da.sqlParameters);
            }
        }
        /// <summary>
        /// 获得该表单该步序的工作流
        ...
        public DataTable Select(string m_FlowName, List<string> m_FormID, int m_Step)
        {
            using (DataAccess da = new DataAccess())
            {
                string strWhere = " Where IsDeleted=0 And Upper(FlowName)=@FlowName And Step=@Step";
                if (m_FormID.Count > 0)
                {
        ...
        public DataTable GetCurrentWorkFlow(string m_FlowName)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
                da.sqlParameters.Add(da.CreateSqlParameter("@FlowName", m_FlowName.Trim().ToUpper()));
                return da.GetDataTable("SELECT t.* FROM (select FormID,Step,FlowName,Approver,rank()over(partition by FormID order by Step) rownumber from " + tableName + " Where IsDeleted=0 And (Result is null or Result='') And Upper(FlowName)=@FlowName) t where t.rownumber = 1", da.sqlParameters);
            }
        }
        /// <summary>
        /// 获得当前的工作流
        ...
        /// <returns></returns>
        public DataTable GetCurrentWorkFlow(string m_FlowName,string m_Approver)
        {
            using (DataAccess da = new DataAccess())
            {
                da.sqlParameters.Clear();
                ...
                return da.GetDataTable("SELECT t.* FROM (select FormID,Step,FlowName,Approver,rank()over(partition by FormID order by Step) rownumber from " + tableName + " Where IsDeleted=0 And (Result is null or Result='') And Upper(FlowName)=@FlowName) t where t.rownumber = 1 And Approver=@Approver", da.sqlParameters);
            }
        }
        /// <summary>
        /// 获得当前的工作流
        ...
        /// <param name="m_Approver">可审批人</param>
        /// <returns></returns>
        public DataTable GetCurrentWorkFlow(string m_FlowName, string m_FormID, string m_Approver)
        {
            using (DataAccess da = new DataAccess())
            {
                if (!String.IsNullOrWhiteSpace(m_Approver))
                {
                    da.sqlParameters.Clear();
                    da.sqlParameters.Add(da.CreateSqlParameter("@FlowName", m_FlowName.Trim().ToUpper()));
                    da.sqlParameters.Add(da.CreateSqlParameter("@FormID", m_FormID.Trim()));
                    da.sqlParameters.Add(da.CreateSqlParameter("@Approver", m_Approver.Trim()));
                    return da.GetDataTable("SELECT t.* FROM (select FormID,Step,FlowName,Approver,rank()over(partition by FormID order by Step) rownumber from " + tableName + " Where IsDeleted=0 And (Result is null or Result='') And Upper(FlowName)=@FlowName And FormID=@FormID) t where t.rownumber = 1 And Approver=@Approver", da.sqlParameters);
                }
                else
                {
                    da.sqlParameters.Clear();
                    ...
                    da.sqlParameters.Add(da.CreateSqlParameter("@FormID", m_FormID.Trim()));
                    da.sqlParameters.Add(da.CreateSqlParameter("@Approver", m_Approver.Trim()));
                    return da.GetDataTable("SELECT t.* FROM (select FormID,Step,FlowName,Approver,rank()over(partition by FormID order by Step) rownumber from " + tableName + " Where IsDeleted=0 And (Result is null or Result='') And Upper(FlowName)=@FlowName And FormID=@FormID) t where t.rownumber = 1", da.sqlParameters);
                }
            }
        }
        /// <summary>
        ...
        /// <param name="m_Approver">可审批人</param>
        /// <returns></returns>
        public DataTable GetCurrentWorkFlow(string m_FlowName, List<string> m_FormID, string m_Approver)
        {
            using (DataAccess da = new DataAccess())
            {
                if (!String.IsNullOrWhiteSpace(m_Approver))
        ...
        public bool FlowIsFinished(string m_FlowName,string m_FormID)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                object objCount = da.GetScalar("Select Count(1) From " + tableName + " Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And (Result='' Or Result is null) And IsDeleted=0", da.sqlParameters);
        ...
        public bool AllFlowIsApproved(string m_FlowName, string m_FormID)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                object objCount = da.GetScalar("Select Count(1) From " + tableName + " Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And Result<>@Result And IsDeleted=0", da.sqlParameters);
                if (objCount.ToString() == "0")
                    return true;
                else
                    return false;
        ...
        public bool Approve(string m_FlowName,string m_FormID, string m_Approver, string m_Remark,out string m_Message)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                try
                {
                    ...
                    object objStep = da.GetScalar("Select Step From " + tableName + " Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And charindex(@Approver,Approver)>0 And (Result='' Or Result is null) And IsDeleted=0", da.sqlParameters);
                    if (objStep == null)
                    {
                        m_Message = "你无权审批("+ m_FormID.Trim() + ")，请联系管理员！";
                    ...
                    da.ExecuteCommand("Update " + tableName + " Set Result=@Result,Approver=@Approver,ApproverName=(SELECT EmployeeName FROM [HRM].[dbo].[Employee] where EmployeeID=@Approver And IsDeleted=0),ApproveDateTime=GetDate(),LastUpdateUserID=@LastUpdateUserID,LastUpdateDateTime=GetDate(),Remark=@Remark Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And Step=@Step And IsDeleted=0", da.sqlParameters);
            ...
            using (DataAccess da = new DataAccess())
            {
                ...
                try
                {
                    ...
                    { 
                        ...
                        if (objStep == null)
                        {
                            m_Message = "你无权审批("+ m_FormID[i].Trim() + ")，请联系管理员！";
                            da.RollbackTransaction();
                            return false;
                        }
                        da.sqlParameters.Clear();
        ...
        public bool Reject(string m_FlowName, string m_FormID, string m_Approver, string m_Remark, out string m_Message, out int m_Step)
        {
            using (DataAccess da = new DataAccess())
            {
                ...
                try
                {
                    ...
                    da.sqlParameters.Add(da.CreateSqlParameter("@LastUpdateUserID", m_Approver));
                    da.sqlParameters.Add(da.CreateSqlParameter("@Remark", m_Remark.Trim()));
                    da.ExecuteCommand("Update " + tableName + " Set Approver=@Approver,ApproverName=(SELECT EmployeeName FROM [HRM].[dbo].[Employee] where EmployeeID=@Approver And IsDeleted=0),Result=@Result,ApproveDateTime=GetDate(),LastUpdateUserID=@LastUpdateUserID,LastUpdateDateTime=GetDate(),Remark=@Remark Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And Step=@Step And IsDeleted=0", da.sqlParameters);

                    da.sqlParameters.Clear();
                    da.sqlParameters.Add(da.CreateSqlParameter("@FlowName", m_FlowName.Trim().ToUpper()));
                    da.sqlParameters.Add(da.CreateSqlParameter("@FormID", m_FormID.Trim().ToUpper()));
                    da.sqlParameters.Add(da.CreateSqlParameter("@Step", Convert.ToInt32(objStep)));
                    object objRejectToStep = da.GetScalar("Select RejectToStep From " + tableName + " Where Upper(FlowName)=@FlowName And Upper(FormID)=@FormID And Step=@Step And IsDeleted=0", da.sqlParameters);
